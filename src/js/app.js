 //Initializing all variables required which are imported from the HTML file
 var courtseInstance;
 var loader = $("#loader");
 var content = $("#content");
...
// Initialization function which calls the https provider
  init: function() {
    return App.initWeb3();
  },
  ...
  initWeb3: function() {
    if (typeof web3 !== 'undefined') {
      ...
      // Specify default instance provided by rinkeby which has address to interact with the blockchain if no web3 instance provided
...
//Function to instantiate the contract
  ...
  // Fuction to render the Front-end page
  render: function() {
   /*  var courtseInstance;
    var loader = $("#loader");
    var content = $("#content");
    ...
    web3.eth.getCoinbase(function(err, account) {
      if (err === null) {
        ...
        $("#accountAddress").html("Your Account: " + account);  //Show which account is currently at use
  ...
  // Function to display the course details of the rubric searched
  getResults: function() {
    var flag=0;
      ...
      for (var i = 0; i<no_courses; i++) {
        courtseInstance.courses(i).then(function(course) {
          ...
          flag = 1;
		      content.show();   //Shows the course details of rubric being searched
          no_content.hide();  
		  }
    ...
    if(flag == 0){
      no_content.show();  //Shows "Course does not exist" if the searched rubric is not present in the blochchain
    }
 ...
 /*  removeadmin: function() {
  ...
  // Function to interact with contract's addCourse function from the HTML page
  addCourse: function() {
    no_content.hide()
    // Check if all the detailas are provided and throw an alert if not
    ...
    document.getElementById("Capacity").value="";
      return instance.addCourse(rubric1,name1,instructor1,venue1,capacity1); //Call the add course function in the contract
	  ...
	  accountinfo.show();
        ...
        if(err.toString().indexOf("Course already exists!!") != -1) {
          alert("Course already exists!!!");  // Show alert if course already exists
          no_content.hide();
      }
